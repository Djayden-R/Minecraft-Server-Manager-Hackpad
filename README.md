...
Couldn't I just make a Discord bot to ping me when my friends go online? Yeah, but it wouldn't be as fun as making a special device that sits on my desk and shows the Minecraft chat live.

## How does it work?
The main logic of checking the server status and the chat isn't done on the Hackpad itself, but instead on the computer it is connected to. To make this work the pc pc will need to run the ```host.py```. This is going to connect with Home Assistant and will gather and process all the information. The program can send the following to the Hackpad (over USB):
- LED:{color}
- CHAT:{person}-{message}
- PLAYERS:{amount}
- MODE:{mode}
